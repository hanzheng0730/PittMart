		...
		//we have created a cookie in login_form.php page so if that cookie is available means user is not login
			if (isset($_COOKIE["product_list"])) {
				...
				for ($i=0; $i < count($product_list); $i++) { 
					...
					$verify_cart = "SELECT id FROM cart WHERE c_id = $_SESSION[uid] AND p_id = ".$product_list[$i];
					...
					if(mysqli_num_rows($result) < 1){
						...
						$delete_existing_product = "DELETE FROM cart WHERE c_id = -1 AND ip_add = '$ip_add' AND p_id = ".$product_list[$i];
			...
			echo "<span style='color:red;'>Wrong email or password. $password</span>";
			exit();
		}
